...
mercury_lamp$`average_div` <- (mercury_lamp$`forword` + mercury_lamp$`reverse`) / 2
...
neon_lamp$`average_div` <- (neon_lamp$`forword` + neon_lamp$`reverse`) / 2
...
spline_model <- splinefun(mercury_lamp$average_div, mercury_lamp$wavelength_nm)

target_wavelengths <- spline_model(neon_lamp$`average_div`)
...
for (match in matched_results) {
  line1 <- match$max_line
  line2 <- match$min_line
  
  d_f_div <- (line1$forword - line2$forword) / (line2$wavelength_nm - line1$wavelength_nm)
...
\begin{align*}
  \text{По формуле \eqref{eq:n1:1}: } \upsilon &= \lambda\nu = 0.34 \cdot 1000 = 340.0 \text{ м/с} \\
\end{align*}
...
ggplot(mercury_lamp, aes(x = wavelength_nm, y = average_div)) +
